...
class Net(nn.Module):
    def __init__(self, kernel_size):
        ...
        kernel = kernel.unsqueeze(0).unsqueeze(0).to(device=gpu)
...
class Matching(object):
    def __init__(self, left_img="/home/tjh/dataset/msl/left_nav_pair/10268.jpg",
                 ...
                 max_vertical_shift=1, min_vertical_shift=1, block_size=3):
    ...
    def main(self):
        ...
        self.left_img_padded = torch.from_numpy(self.left_img_padded).to(device=gpu, dtype=torch.float16)
        self.right_img_padded = torch.from_numpy(self.right_img_padded).to(device=gpu, dtype=torch.float16)
        ...
        # self.left_img_conved = self.left_img_conved.to(device=gpu)
        # self.right_img_conved = self.right_img_conved.to(device=gpu)
    ...
    def matching(self, left_img_idxes):
        disparity = []
        for left_img_idx in left_img_idxes:
            ...
            matched_column = search_pixels[costs.index(min(costs))][1]
